                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                          <Card className="bg-primary/5">
                              <CardHeader className="pb-2">
                                  ...
                                  <CardTitle className="text-2xl">
                                    ...
                                    }).format(Number(report.bankStatementBalance || 0)) : Number(report.bankStatementBalance || 0)}
                                  </CardTitle>
                              </CardHeader>
                          </Card>
                          <Card className="bg-green-50">
                              <CardHeader className="pb-2">
                                  ...
                                  <CardTitle className="text-2xl text-green-700">+{new Intl.NumberFormat('en-US').format(Number(report.totalInTransit || 0))}</CardTitle>
                              </CardHeader>
                          </Card>
                          <Card className="bg-red-50">
                              <CardHeader className="pb-2">
                                  <CardDescription>Unpresented Checks</CardDescription>
                                  <CardTitle className="text-2xl text-red-700">-{new Intl.NumberFormat('en-US').format(Number(report.totalUnpresented || 0))}</CardTitle>
                              </CardHeader>
                          </Card>
                          <Card className={cn(Number(report.difference || 0) == 0 ? "bg-blue-50" : "bg-orange-50")}>
                              <CardHeader className="pb-2">
                                  <CardDescription>GL Balance</CardDescription>
                                  <CardTitle className="text-2xl">{new Intl.NumberFormat('en-US').format(Number(report.glBalance || 0))}</CardTitle>
                              </CardHeader>
                          </Card>
                      </div>
                      ...
                      <div className="border rounded-lg p-6 space-y-4">
                          <div className="flex justify-between border-b pb-2">
                              ...
                              <span className="font-mono">{new Intl.NumberFormat('en-US').format(Number(report.bankStatementBalance || 0))}</span>
                          </div>
                          <div className="flex justify-between border-b pb-2 text-green-600">
                              <span>Add: Deposits in Transit (not yet in bank)</span>
                              <span className="font-mono">+{new Intl.NumberFormat('en-US').format(Number(report.totalInTransit || 0))}</span>
                          </div>
                          <div className="flex justify-between border-b pb-2 text-red-600">
                              <span>Less: Unpresented Checks (not yet cleared bank)</span>
                              <span className="font-mono">-{new Intl.NumberFormat('en-US').format(Number(report.totalUnpresented || 0))}</span>
                          </div>
                          <div className="flex justify-between border-b pb-2 font-bold">
                              <span>Adjusted Bank Balance</span>
                              <span className="font-mono">{new Intl.NumberFormat('en-US').format((Number(report.bankStatementBalance) || 0) + (Number(report.totalInTransit) || 0) - (Number(report.totalUnpresented) || 0))}</span>
                          </div>
                          <div className="flex justify-between pt-4 font-bold text-lg">
                              <span>Difference to GL</span>
                              <span className={cn("font-mono", Number(report.difference || 0) == 0 ? "text-green-600" : "text-red-600")}>
                                  {new Intl.NumberFormat('en-US').format(Number(report.difference || 0))}
                              </span>
                          </div>
                    </div>
                  </div>
