...
export default function FinancialPerformancePage() {
  ...
  const fetchReport = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/reports/financial-performance?startDate=${startDate}&endDate=${endDate}&currency=${reportingCurrency}`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
      const result = await response.json();
      setData(result);
    } catch (error) {
      toast.error("Failed to fetch Financial Performance");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleEmailReport = async () => {
    try {
      const response = await fetch("/api/reports/email", {
        ...
        body: JSON.stringify({
          reportName: "Statement of Financial Performance",
          endpoint: `/api/reports/financial-performance?startDate=${startDate}&endDate=${endDate}&currency=${reportingCurrency}`,
          summary: {
            "Period": `${format(new Date(startDate), "PP")} to ${format(new Date(endDate), "PP")}`,
            "Currency": reportingCurrency,
            "Total Revenue": `${reportingCurrency} ${parseFloat(data.summary.totalRevenue).toLocaleString()}`,
            "Total Expenses": `${reportingCurrency} ${parseFloat(data.summary.totalExpenses).toLocaleString()}`,
            "Net Surplus/(Deficit)": `${reportingCurrency} ${parseFloat(data.summary.surplus).toLocaleString()}`,
          }
        })
      });
          ...
          <div>
            ...
            {trend !== undefined && (
              <div className="flex items-center gap-1 mt-2">
                ...
                <span className={cn("text-xs font-semibold", parseFloat(trend) >= 0 ? "text-emerald-600" : "text-rose-600")}>
                  {trend}% margin
                </span>
              </div>
            )}
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-amber-50 text-amber-600 rounded-full border border-amber-100">IPSAS 1</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <ReportToolbar 
            reportName="Financial Performance" 
            endpoint="/api/reports/financial-performance" 
          ...
          <div className="flex flex-wrap items-end gap-6">
            <div className="space-y-2">
              ...
              <Input 
                ...
                className="w-[180px] border-slate-200 focus:ring-amber-500 focus:border-amber-500"
              />
            </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-slate-500 font-medium animate-pulse">Calculating institutional performance...</p>
        </div>
      ) : data ? (
        <div ref={printRef} className="space-y-8 print:p-8 print:bg-white print:text-black print:m-0 landscape-container">
          ...
          <div className="hidden print:block mb-10 border-b-2 border-slate-900 pb-6">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Statement of Financial Performance</h1>
              ...
              <div className="text-right">
                ...
                <p className="text-xs font-bold text-amber-600 mt-1 uppercase">IPSAS COMPLIANT</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4">
            ...
            <SummaryCard 
              ...
              value={data.summary.surplus} 
              icon={Building2} 
              trend={data.summary.margin ? parseFloat(data.summary.margin).toFixed(1) : "0"}
              colorClass={parseFloat(data.summary.surplus) >= 0 ? "bg-indigo-50 text-indigo-600" : "bg-rose-50 text-rose-600"}
            />
            <Card className="overflow-hidden border-slate-200/60 shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-500 mb-1 uppercase tracking-wider">Efficiency Ratio</p>
                    <h3 className="text-2xl font-bold text-slate-900 tracking-tight">
                      {data.summary.totalRevenue > 0 
                        ? (Number(data.summary.totalExpenses) / Number(data.summary.totalRevenue) * 100).toFixed(1) 
                        : "0"}%
                    </h3>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 print:grid-cols-2">
            <Card className="border-slate-200/60 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <div>
                  <CardTitle className="text-base font-bold text-slate-800">Revenue vs Expenditure</CardTitle>
                  <CardDescription>Fiscal volume comparison</CardDescription>
                </div>
                <PieChartIcon className="w-5 h-5 text-slate-400" />
              </CardHeader>
              <CardContent className="h-[300px] mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.chartData.revenueVsExpense}>
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                      {data.chartData.revenueVsExpense.map((entry: any, index: number) => (
            ...
            <Card className="border-slate-200/60 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <div>
                  <CardTitle className="text-base font-bold text-slate-800">Expense Composition</CardTitle>
          ...
          <div className="hidden print:block mt-12 pt-8 border-t border-slate-200">
            ...
            <div className="mt-12 text-center">
              <p className="text-[9px] text-slate-400 uppercase tracking-[0.2em]">Generated by IPSAS Accounting System • Institutional Performance Audit Document</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="text-center py-24 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-lg font-bold text-slate-900">No Performance Data</h3>
          <p className="text-slate-500 max-w-xs mx-auto mt-2">We couldn't find any financial activities for the selected period. Please ensure your transactions are posted.</p>
        </div>
      )}
